  ...
  return (
    <div className="fixed bottom-6 right-6 z-30">
      <div className="relative">
        ...
        <button onClick={() => setAbierto((v) => !v)} className="rounded-full p-4 shadow-lg bg-black/80 text-white dark:bg-white/15 backdrop-blur hover:scale-105 transition">
          <Plus className={`size-6 transition ${abierto ? 'rotate-45' : 'rotate-0'}`} />
        </button>
      </div>
    </div>
  );
